"""main.py is a standalone script that analyses the data from the simulator-
and displays the days and hours with the most passings.
...
and the simulation is of such big scale. While this does add some run time
to the scripts i believe its cooler to have it be a little slower and more realistic. 
"""
import time 
import sys 
import json
...
def see_help_main():
    """
    Simple function that asks the user if they would like to see help for-
    the classes in main.py
    """
    valid = False
    while valid == False:
...
class Display():
    """
    Class that displays the day(s) that had the most passings past the registering point-
    and the hour(s) that had the most passings in those day(s)

    <sourceFile> is a string of the file path for the data gathered in the simulation 
    from simulator.py
    """
    def __init__(
        self,
    ...
    def day(self):
        ...
        Simply put it takes all the total passings of each day, adds that-
        data to a list, and finds the max value and its associated date(s).
        """
        with open(self.sourceFile, 'r') as dataFile:
            ...
            while True:
                ...
                max_day_data = daysData[max_day_index]

                max_days.append(max_day_data)

                del days[max_day_index]
                del daysData[max_day_index]
            ...
            for i in range(len(max_days)):
                print(f'\t{max_days[i]["month"]} {max_days[i]["day"]}. {max_days[i]["year"]}')
    ...
    def hour(self):
        ...
        a list and finding the max value and its associated hour in that list. 
        """
        if days_known == False:
            Display.day()
        with open(self.sourceFile, 'r') as dataFile:
            ...
            for i in range(0,len(max_days)):
                ...
                for j in range(24):
                    hour_total = max_day[max_days[i]["day"]][j][f"{j}"][-1]["total"]
                ...
                print(f'\nDen {max_days[i]["day"]}. {max_days[i]["month"]}  {max_days[i]["year"]} var timen(e) med flest passeringer:\n')
...
if __name__ == "__main__":
    display = Display(
        sourceFile = CURRENT_DIR + "\\simulator\\data.json"
    )
    register = Register(
    ...
    while valid == False:
        simulate = str(input(f"\nSimulate cars passing registering point for ({simulator.years}) year(s)? [y/n] : "))
